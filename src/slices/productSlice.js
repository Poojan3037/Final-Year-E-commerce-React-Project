...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setProductsOnLoad(state, action) {
      ...
      state.copyProducts = action.payload.slice(0, 9);
    },
    ...
    getWomenClothes(state, action) {
      let womenClothes = state.products
        .filter((item) => {
          if (item.gender === "female") {
            return item;
          }
